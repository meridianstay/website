...
export function ReviewForm({ slug, bookingCode, onPosted }: { slug: string; bookingCode: string; onPosted: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (comment.trim().length < 10) return setError(t('review.min10'))
    setSending(true)
    setError(null)
    try {
  ...
  return (
    <form onSubmit={submit} className="bg-brand-50/60 border border-brand-100 rounded-2xl p-5 space-y-3">
      <p className="font-bold text-slate-900 text-sm">{t('review.how')}</p>
      ...
      <textarea id="review-comment" rows={3} value={comment} onChange={(e) => setComment(e.target.value)} maxLength={2000} placeholder={t('review.placeholder')} className="w-full bg-white border border-slate-200 rounded-2xl p-3 text-sm focus:outline-none focus:border-brand-500" />
      ...
      <button type="submit" disabled={sending} className="bg-slate-900 disabled:bg-slate-400 text-white text-xs font-bold py-2.5 px-5 rounded-xl">
        {sending ? t('review.posting') : t('review.post')}
      </button>
    </form>
  )
}
