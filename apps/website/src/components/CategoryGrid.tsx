  ...
  return (
    <section className="max-w-[1180px] mx-auto px-5 pt-14 pb-12">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link to="/search" className="text-xs font-semibold text-brand-600 hover:text-brand-700 flex items-center space-x-1">
        ...
        {categories.map((cat, i) => (
          ...
          >
            ...
            <div className="absolute bottom-4 left-4 right-4 text-white">
              <span className={`inline-block px-2.5 py-0.5 text-[9px] font-bold uppercase rounded-full mb-1 ${cat.tagTone === 'green' ? 'bg-brand-500 text-white' : 'bg-brand-yellow-500 text-slate-900'}`}>
                {cat.tag}
              </span>
