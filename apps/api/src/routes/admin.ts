...
const adminId = (c: Parameters<typeof currentUser>[0]) => currentUser(c).id
...
adminRoutes.post('/admin/listings/:id/approve', async (c) => (await adminService.approveListing(adminId(c), Number(c.req.param('id'))), done()))
adminRoutes.post('/admin/listings/:id/reject', async (c) => (await adminService.rejectListing(adminId(c), Number(c.req.param('id')), str((await body(c)).reason)), done()))
adminRoutes.post('/admin/listings/:id/feature', async (c) => {
  const rank = (await body(c)).rank
  await adminService.featureListing(adminId(c), Number(c.req.param('id')), rank === null ? null : Number(rank))
  return done()
})

// ─── Bookings ────────────────────────────────────────────────────────────────
adminRoutes.get('/admin/bookings', async (c) => c.json({ bookings: await adminService.listBookings(q(c.req.query('q'))) }))
adminRoutes.post('/admin/bookings/:code/cancel', async (c) => (await adminService.cancelBooking(adminId(c), c.req.param('code')), done()))
...
adminRoutes.patch('/admin/users/:id', async (c) => {
  ...
  await adminService.updateUser(adminId(c), Number(c.req.param('id')), {
...
adminRoutes.post('/admin/reviews/:id/hide', async (c) => (await reviewService.setHidden(adminId(c), Number(c.req.param('id')), true), done()))
adminRoutes.post('/admin/reviews/:id/restore', async (c) => (await reviewService.setHidden(adminId(c), Number(c.req.param('id')), false), done()))
...
adminRoutes.patch('/admin/messages/:id', async (c) => (await adminService.setMessageStatus(adminId(c), Number(c.req.param('id')), str((await body(c)).status)), done()))

// ─── Website content ─────────────────────────────────────────────────────────
adminRoutes.get('/admin/settings', async (c) => c.json(await contentRepo.settings()))
adminRoutes.put('/admin/settings/:key', async (c) => {
  const key = c.req.param('key')
  return c.json({ [key]: await contentService.saveSetting(adminId(c), key, await body(c)) })
})
adminRoutes.get('/admin/pages', async (c) => c.json({ pages: await contentRepo.allPages() }))
adminRoutes.put('/admin/pages/:slug', async (c) => (await contentService.savePage(adminId(c), c.req.param('slug'), await body(c)), done()))
adminRoutes.delete('/admin/pages/:slug', async (c) => (await contentService.deletePage(adminId(c), c.req.param('slug')), done()))
...
adminRoutes.post('/admin/db/tables/:table/rows', async (c) => c.json({ row: await explorerService.insert(adminId(c), c.req.param('table'), await body(c)) }, 201))
adminRoutes.patch('/admin/db/tables/:table/rows', async (c) => {
  ...
  return c.json({ row: await explorerService.update(adminId(c), c.req.param('table'), rowKey(JSON.stringify(b.key)), changes) })
})
adminRoutes.delete('/admin/db/tables/:table/rows', async (c) => (await explorerService.remove(adminId(c), c.req.param('table'), rowKey(c.req.query('key'))), done()))
